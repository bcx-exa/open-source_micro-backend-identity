...
async function run() {
  try {
    ...
    const getDomainRecord = await ses.getSesDomainRecords(
    ...
    const verifyDomain = await ses.createRoute53Record(
      recordName,
      recordValue,
      "TXT"
    );
    ...
    for (let index = 0; index < getDkimRecords.DkimTokens.length; index++) {
      ...
      const dRecordName =
        getDkimRecords.DkimTokens[index] +
        "._domainkey." +
        process.env.API_DOMAIN;
      const dRecordValue =
        getDkimRecords.DkimTokens[index] + ".dkim.amazonses.com";

      console.log("name:", dRecordName);
      console.log("value:", dRecordValue);
      const verifyDkim = await ses.createRoute53Record(
        dRecordName,
        dRecordValue,
        "CNAME"
      );
