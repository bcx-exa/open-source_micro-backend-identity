...
async function issueTokens(user_id, client_id, done) {
  try {
    ...
    const idToken: Oauth = {
      ...
      token_link: tokens.id_token.token_link,
      token_type: 'id_token',
      user: user,
      client: client,
      disabled: false,
    ...
    const accessToken: Oauth = {
      ...
      token_link: tokens.access_token.token_link,
    ...
    const refreshToken: Oauth = {
      ...
      token_link: tokens.refresh_token.token_link,
...
server.exchange(oauth2orize.exchange.refreshToken(async (client, refreshToken, _scope, done) => {
  ...
  const dbIdToken = await oRepository.find({ token_link: tokenDecoded.token_link, token_type: 'id_token' });
  ...
  const dbAccessToken = await oRepository.find({ token_link: tokenDecoded.token_link, token_type: 'access_token' });
  ...
  const dbRefreshToken = await oRepository.find({ token: refreshToken, token_type: 'refresh_token' });
